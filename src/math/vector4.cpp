...
namespace voxyl::math {
    ...
    Vector4 Vector4::operator+(const Vector4& vector) const { return Vector4(x + vector.x, y + vector.y, z + vector.z, w + vector.w); }
    ...
    Vector4 Vector4::normalized() const { float len = length(); assert(len > 0.0f); return *this / len; }
    Vector4 Vector4::absolute() const { return Vector4(std::fabs(x), std::fabs(y), std::fabs(z), std::fabs(w)); }
    ...
    bool Vector4::approximately(const Vector4& vector, float epsilon) const {
    ...
    Vector4 Vector4::minimum(const Vector4& alpha, const Vector4& beta) {
        return Vector4(std::fmin(alpha.x, beta.x), std::fmin(alpha.y, beta.y), std::fmin(alpha.z, beta.z), std::fmin(alpha.w, beta.w));
    }
    Vector4 Vector4::maximum(const Vector4& alpha, const Vector4& beta) {
        return Vector4(std::fmax(alpha.x, beta.x), std::fmax(alpha.y, beta.y), std::fmax(alpha.z, beta.z), std::fmax(alpha.w, beta.w));
    }
