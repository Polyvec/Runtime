...
namespace voxyl::math {
    ...
    Vector3 Vector3::operator+(const Vector3& vector) const { return Vector3(x + vector.x, y + vector.y, z + vector.z); }
    ...
    Vector3 Vector3::operator/(float scalar) const { assert(scalar != 0.0f); return Vector3(x / scalar, y / scalar, z / scalar); }
    ...
    Vector3 Vector3::absolute() const { return Vector3(std::fabs(x), std::fabs(y), std::fabs(z)); }
    ...
    Vector3 Vector3::minimum(const Vector3& alpha, const Vector3& beta) {
        return Vector3(std::fmin(alpha.x, beta.x), std::fmin(alpha.y, beta.y), std::fmin(alpha.z, beta.z));
    }
    Vector3 Vector3::maximum(const Vector3& alpha, const Vector3& beta) {
        return Vector3(std::fmax(alpha.x, beta.x), std::fmax(alpha.y, beta.y), std::fmax(alpha.z, beta.z));
    }
